...
export default function MoreImagesLoad() {
  ...
  const fetchData = useCallback(async pageNumber => {
    const api=`https://jsonplaceholder.typicode.com/photos?page=${pageNumber}&limit=15`
        ...
        <ul className="imageGrid">
          {images.map((image, index) => (
            <li key={index} className="imageContainer">
              <img src={image.thumbnailUrl} alt={image.author} className="imageStyle" />
            </li>
          ))}
        </ul>
      )}
